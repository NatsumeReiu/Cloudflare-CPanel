...
It seems that CloudFlare doesn't maintain the plugin anymore. The original script cannot work due to github URL change. This fork has fixed that.
...
`bash <(curl -s https://raw.githubusercontent.com/NatsumeReiu/CloudFlare-CPanel/master/cloudflare.install.sh) -k [YOUR_HOST_API_KEY] -n '[YOUR_COMPANY_NAME]' `
...
`bash <(curl -s https://raw.githubusercontent.com/NatsumeReiu/CloudFlare-CPanel/master/cloudflare.uninstall.sh) && rm -f cloudflare.uninstall.sh`
